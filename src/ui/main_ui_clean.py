...
class ApiTestPage:
    ...
    def __init__(self, page: ft.Page):
        ...
        self.tab_bar_row = ft.Row(controls=[], spacing=5)  # Tab栏UI组件
    ...
    def _on_tab_url_change(self, e):
        ...
        if self.current_tab_index >= 0 and self.current_tab_index < len(self.request_tabs):
            self.request_tabs[self.current_tab_index].is_modified = True
            self._update_tab_bar()
            # 调用URL自动解析逻辑
    ...
    def _build_main_content(self) -> ft.Container:
            ...
            content=ft.Column(
                controls=[
                    ...
                    ft.Container(
                        content=ft.Column(
                            controls=[
                                ft.Text("响应", size=16, weight=ft.FontWeight.BOLD),
                                self.response_panel,
                            ],
                            spacing=10,
                        ),
        ...
        self.url_input = ft.TextField(
            ...
            text_size=14,
            label_style=ft.TextStyle(size=13, weight=ft.FontWeight.BOLD),
            on_change=self._on_url_change,  # 添加URL变化事件
        )
    ...
    def _build_request_tabs(self) -> ft.Tabs:
        """构建请求配置Tab控件(Params/Headers/Body)"""
        # 按照RULES.md规范：ft.Tab的content需要在创建后单独设置
        tab_params = ft.Tab(label="Params")
        tab_params.content = ft.Container(
            content=self.params_list,
            padding=10,
        )

        tab_headers = ft.Tab(label="Headers")
        tab_headers.content = ft.Container(
            content=self.headers_list,
            padding=10,
        )

        tab_body = ft.Tab(label="Body")
        tab_body.content = ft.Container(
            content=self.body_editor,
            padding=10,
        )

        # 按照RULES.md规范：ft.Tabs必须提供length参数
        return ft.Tabs(
            selected_index=0,
            length=3,
            content=ft.Row(controls=[tab_params, tab_headers, tab_body]),
            expand=True,
        )
